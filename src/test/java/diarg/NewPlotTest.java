...
/**
 * This class contains code to generate value-based agreement scenarios and their expansions to then measure
 * value impacts and changes in degrees of agreement. The results are printed out as Python variable, so that
 * the data can be plotted in Python/Jupyter notebooks. Note that the flag ``topicFixed'' in the first test defines
 * whether topics should be expanded as the agreement scenarios are expanded. Similarly, expanding or non-expaning
 * topics for value impact experiments can be managed in line 350 (see the comment there).
 */

public class NewPlotTest {
    ...
    void generatePlotDataDegrees() {
        ...
        List<Double> normalizedMinDegrees = new ArrayList<>();
        List<Double> normalizedMedDegrees = new ArrayList<>();
        List<Double> normalizedMeanDegrees = new ArrayList<>();
        // Flip this flag, depending on whether we want to expand the topic or keep it fixed
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            double normalizedMinDegree = 0d;
            double normalizedMedianDegree = 0d;
            double normalizedMeanDegree = 0d;
            for (int j = 0; j < sample_per_iteration; j++) {
                ...
                // Log maximal changes that would be possible
                double maxChangeMin = minimalAgreementDegree > 0.5 ? minimalAgreementDegree : 1 - minimalAgreementDegree;
                double maxChangeMed = medianAgreementDegree > 0.5 ? medianAgreementDegree : 1 - medianAgreementDegree;
                double maxChangeMean = meanAgreementDegree > 0.5 ? meanAgreementDegree : 1 - meanAgreementDegree;
                // Normalize changes so that 1 is the maximum change that would be possible
                normalizedMinDegree += Math.abs(minimalAgreementDegree - newMinimalAgreementDegree) / maxChangeMin;
                normalizedMedianDegree += Math.abs(medianAgreementDegree - newMedianAgreementDegree) / maxChangeMed;
                normalizedMeanDegree += Math.abs(meanAgreementDegree - newMeanAgreementDegree) / maxChangeMean;
            ...
            normalizedMinDegrees.add(normalizedMinDegree / sample_per_iteration);
            normalizedMedDegrees.add(normalizedMedianDegree / sample_per_iteration);
            normalizedMeanDegrees.add(normalizedMeanDegree / sample_per_iteration);
        }
        ...
        System.out.println("normalized_min_degrees = " + normalizedMinDegrees.toString());
        System.out.println("normalized_med_degrees = " + normalizedMedDegrees.toString());
        System.out.println("normalized_mean_degrees = " + normalizedMeanDegrees.toString());
    }
    ...
    void generatePlotDataImpact() {
        ...
        List<Double> normalizedMinValueImpact = new ArrayList<>();
        List<Double> normalizedMeanValueImpact = new ArrayList<>();
        List<Double> normalizedMedValueImpact = new ArrayList<>();
        int iterations = 15;
        int sample_per_iteration = 30;
        for (int i = 0; i < iterations; i++) {
            Collection<Argument> topic = new ArrayList<>();
            double agrMinImpact = 0d;
            double agrMedianImpact = 0d;
            double agrMeanImpact = 0d;
            double normalizedAgrMinImpact = 0d;
            double normalizedAgrMedianImpact = 0d;
            double normalizedAgrMeanImpact = 0d;
            for (int j = 0; j < sample_per_iteration; j++) {
                ...
                double minImpact = Math.abs(vbScenario0.determineMinimalAgreementImpact(impactValue));
                double medianImpact = Math.abs(vbScenario0.determineMedianAgreementImpact(impactValue));
                double meanImpact = Math.abs(vbScenario0.determineMeanAgreementImpact(impactValue));
                agrMinImpact += minImpact;
                agrMedianImpact += medianImpact;
                agrMeanImpact += meanImpact;

                // Log maximal changes that would be possible
                double minDegree = vbScenario0.determineMinimalAgreement();
                double medianDegree = vbScenario0.determineMedianAgreement();
                double meanDegree = vbScenario0.determineMeanAgreement();
                double maxChangeMin = minDegree > 0.5 ? minDegree : 1 - minDegree;
                double maxChangeMed = medianDegree > 0.5 ? medianDegree : 1 - medianDegree;
                double maxChangeMean = meanDegree > 0.5 ? meanDegree : 1 - meanDegree;
                // Normalize changes so that 1 is the maximum change that would be possible
                normalizedAgrMinImpact += minImpact / maxChangeMin;
                normalizedAgrMedianImpact += medianImpact / maxChangeMed;
                normalizedAgrMeanImpact += meanImpact / maxChangeMean;
            }
            ...
            normalizedMinValueImpact.add(normalizedAgrMinImpact / sample_per_iteration);
            normalizedMedValueImpact.add(normalizedAgrMedianImpact / sample_per_iteration);
            normalizedMeanValueImpact.add(normalizedAgrMeanImpact / sample_per_iteration);
        }
        ...
        System.out.println("normalized_min_impacts = " + normalizedMinValueImpact.toString());
        System.out.println("normalized_med_impacts = " + normalizedMedValueImpact.toString());
        System.out.println("normalized_mean_impacts = " + normalizedMeanValueImpact.toString());
    }
}
